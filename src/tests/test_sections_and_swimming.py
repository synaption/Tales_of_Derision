...
def _world(player_x: int, player_y: int, width: int = 120, height: int = 60):
    ...
    ecs.add_processor(render_processor, priority=0)
...
def test_crossing_a_section_edge_snaps_camera_and_logs_transition() -> None:
    ...
    ecs.process("move_right")  # 79 -> 80 crosses into the right column
...
def test_swimming_status_shows_own_tile_then_water_on_a_timed_cycle() -> None:
    ...
    swimmer_ent = ecs.create_entity(Position(61, 30), Renderable("@"))
    swimmer = ecs.component_for_entity(swimmer_ent, Position)
    ...
    on_land_ent = ecs.create_entity(Position(60, 30), Renderable("@"))
    on_land = ecs.component_for_entity(on_land_ent, Position)
...
def test_statuses_stack_sequentially_swim_then_fire() -> None:
    ...
    game_map, _r, rp, _p = _world(60, 30)
    ent = ecs.create_entity(Position(61, 30), Renderable("@"), OnFire())
