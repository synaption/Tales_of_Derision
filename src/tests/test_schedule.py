...
def test_ties_break_by_entity_id_deterministically() -> None:
    first = ecs.create_entity()
    second = ecs.create_entity()
...
def test_complete_action_advances_by_baseline_cost() -> None:
    ...
    assert ecs.component_for_entity(ent, Actor).last_acted == 300
...
def test_quicker_actor_takes_more_turns_over_a_span() -> None:
    slow = ecs.create_entity(Attributes(dexterity=10))
    fast = ecs.create_entity(Attributes(dexterity=25))
...
def test_scheduler_sequence_is_deterministic() -> None:
    ...
    def build_and_run() -> list[int]:
        ecs.clear_database()
        a = ecs.create_entity(Attributes(dexterity=12))
        b = ecs.create_entity(Attributes(dexterity=17))
