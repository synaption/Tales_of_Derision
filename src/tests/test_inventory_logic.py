...
def test_trade_can_move_equipped_item_between_actors() -> None:
    player_ent = ecs.create_entity(
        Name("You"),
        Inventory(items=[]),
        Equipment(slots={"main hand": "Rusty Sword"}),
    )
    npc_ent = ecs.create_entity(
        Name("Villager"),
        Inventory(items=[]),
    ...
    player_equipment = ecs.component_for_entity(player_ent, Equipment)
    npc_inventory = ecs.component_for_entity(npc_ent, Inventory)
...
def test_find_interaction_corpse_supports_directional_and_idle_lookup() -> None:
    ...
    corpse_here = ecs.create_entity(
        Position(5, 5),
        Corpse(),
        Name("Corpse Here"),
        Inventory(items=["Coin"]),
    )
    corpse_right = ecs.create_entity(
        Position(6, 5),
        Corpse(),
        Name("Corpse Right"),
        Inventory(items=["Pebble"]),
    )
    corpse_up = ecs.create_entity(
        Position(5, 4),
        Corpse(),
        Name("Corpse Up"),
